...
#         self.touch_flash_frames = 6
#         self.last_touch_t       = -1e9
#         self.min_touch_interval = 0.15
#         self.vy_min_flip        = 100.0
#         self.prev_det           = None
#         self.prev_vy            = None
...
#     # ------------------------------------------------------------------
#     # F3: skill score overlay on frame
#     # ------------------------------------------------------------------

#     def _draw_skill_overlay(self, frame, skill_score, tips):
#         """Draw skill score and first coaching tip on frame."""
#         h, w = frame.shape[:2]

#         # Skill score — top right corner
#         score_color = (
#             (0, 200, 0)     if skill_score >= 70 else
#             (0, 165, 255)   if skill_score >= 40 else
#             (0, 0, 255)
#         )
#         cv2.putText(frame,
#                     f"Skill: {skill_score:.0f}/100",
#                     (w - 280, 60),
#                     cv2.FONT_HERSHEY_SIMPLEX, 0.9, score_color, 2)

#         # First coaching tip — bottom center
#         if tips:
#             tip_text = tips[0]
#             text_w   = cv2.getTextSize(
#                 tip_text, cv2.FONT_HERSHEY_SIMPLEX, 0.55, 1)[0][0]
#             tip_x    = max(10, (w - text_w) // 2)
...
#         # Drawing order: trail → pose → ball → overlays
#         self._draw_trail(frame)
#         self._draw_pose(frame, landmarks_px)
...
#         # F3 skill overlay — compute live every frame
#         if len(self.touches) >= 2:
#             self._compute_peak_and_drift()
#             skill_score, tips, _ = self._compute_skill_score()
#             self._draw_skill_overlay(frame, skill_score, tips)
...
#         """
#         For each pair of consecutive touches compute:
#         - peak height (pixels ball rose between touches)
#         - horizontal drift
#         - time interval
#         """
#         if len(self.touches) < 2:
...
#             ]
#             if between:
#                 self.peaks.append(max(p["y"] for p in between) -
#                                   min(p["y"] for p in between))
#             else:
#                 self.peaks.append(0)

#     def _compute_rhythm_score(self):
#         """Rhythm = consistency of touch intervals. 0-100."""
...
#               f"{'Yes' if self.last_landmarks else 'NONE — pose never detected!'}")
...
#     for video_path in videos:
#         out_path = output_dir / f"{video_path.stem}_F3_annotated.mp4"
#         print(f"\nProcessing: {video_path.name}")
#         try:
#             vp.process_video(video_path, out_path)
#             print(f"Saved → {out_path.name}")
...
#     def reset_tracking(self):
#         """Reset all state — called at start of every video."""
#         self.kf = cv2.KalmanFilter(4, 2)
#         self.kf.measurementMatrix = np.array(
#             [[1, 0, 0, 0],
#              [0, 1, 0, 0]], dtype=np.float32)
...
#         # Detected-only history (metrics only)
#         self.ball_history_detected = []
...
#         # Milestone F1 — foot identification
#         self.last_landmarks     = None

#         # Milestone F2 — proximity gate
#         self.FOOT_PROXIMITY_PX  = 400

#         # Milestone F3 — peak / drift / rhythm
#         self.peaks              = []
#         self.drifts             = []
#         self.intervals          = []
...
#     def _kalman_predict(self):
#         """Safe — always flattens 2D output."""
#         pred = self.kf.predict()
#         pred = np.array(pred).flatten()
#         return int(pred[0]), int(pred[1])

#     def _kalman_state(self):
#         """Safe read of statePost without predicting."""
#         flat = np.array(self.kf.statePost).flatten()
#         return int(flat[0]), int(flat[1])
...
#     def _is_ball_near_foot(self, ball_cx, ball_cy):
#         if self.last_landmarks is None:
#             return True  # no pose — don't block
...
#     def _draw_foot_chart(self, frame):
#         """Live L vs R bar chart — bottom left corner."""
#         h, w  = frame.shape[:2]
#         feet  = [t["foot"] for t in self.touches]
#         left  = feet.count("L")
#         right = feet.count("R")
...
#         # Pose — save landmarks every frame
#         landmarks_px = self.detect_pose(frame, timestamp_ms)
#         if landmarks_px is not None:
#             self.last_landmarks = landmarks_px
...
#         # Draw — trail → pose → ball → touch overlay → chart
#         self._draw_trail(frame)
#         self._draw_pose(frame, landmarks_px)
...
#         # L vs R chart on video
#         self._draw_foot_chart(frame)

#         # NOTE: coaching tips / skill score NOT drawn here
#         # All F3 data returned via get_results() for API use only

#         return frame

#     # ------------------------------------------------------------------
#     # Metrics — all for API use
#     # ------------------------------------------------------------------
...
#     def get_results(self):
#         """
#         Call this after process_video() to get full JSON results for API.
#         """
#         summary                  = self.get_foot_summary()
#         best_streak, all_streaks = self._calculate_best_streak()
#         self._compute_peak_and_drift()
...
#     def process_video(self, input_path, output_path):
#         """
#         Process video and return results dict.
#         results = vp.process_video(input, output)  ← use this for API later
#         """
#         self.reset_tracking()
#         self.pose_landmarker = self._create_pose_landmarker()
...
#         return results  # ← ready for FastAPI when you need it
...
#             print(f"Saved → {out_path.name}")
#             # results dict is ready here — pass to FastAPI later
#         except Exception as e:
#             print(f"Failed on {video_path.name}: {e}")
...
#         # Milestone F4 — knee angle tracking
...
#     # ------------------------------------------------------------------
#     # F4: knee angle computation
#     # ------------------------------------------------------------------

#     def _compute_knee_angle(self, landmarks_px, side="left"):
#         """
#         Compute knee bend angle via hip → knee → ankle.
#         180° = straight leg (stiff)
#         90°  = well bent (good posture)
...
#         if any(i >= len(landmarks_px) for i in
#                [hip_idx, knee_idx, ankle_idx]):
#             return None
...
#         # Pose — save landmarks + collect knee angles every frame
#         landmarks_px = self.detect_pose(frame, timestamp_ms)
#         if landmarks_px is not None:
#             self.last_landmarks = landmarks_px

#             # F4: collect knee angles
#             left_angle  = self._compute_knee_angle(landmarks_px, "left")
#             right_angle = self._compute_knee_angle(landmarks_px, "right")
#             if left_angle is not None:
#                 self.knee_angles_left.append(left_angle)
#             if right_angle is not None:
#                 self.knee_angles_right.append(right_angle)

#         # Touch detection — detected frames only
#         if status == "detected" and len(self.ball_history_detected) > 0:
#             self._update_touch_detection(self.ball_history_detected[-1])

#         # Draw order: trail → pose → ball → overlays
#         self._draw_trail(frame)
#         self._draw_pose(frame, landmarks_px)
...
#         # NOTE: coaching tips / skill score / knee feedback
#         # NOT drawn on video — all returned via get_results() for API

#         return frame

#     # ------------------------------------------------------------------
#     # Metrics — all for API use via get_results()
#     # ------------------------------------------------------------------
...
#         """F4: Stiffness proxy from average knee angle."""
#         result = {
...
#     def get_results(self):
#         """
#         Returns full results dict — ready for FastAPI response.
#         Call after process_video().
#         """
#         summary                      = self.get_foot_summary()
#         best_streak, all_streaks     = self._calculate_best_streak()
#         self._compute_peak_and_drift()
...
#     def process_video(self, input_path, output_path):
#         """
#         Process video — returns full results dict.
#         results = vp.process_video(input, output)
#         Wire directly to FastAPI when ready.
#         """
#         self.reset_tracking()
#         self.pose_landmarker = self._create_pose_landmarker()
