...
#         self.min_touch_interval = 0.18
#         self.vy_min_flip        = 250.0
#         self.prev_det           = None
#         self.prev_vy            = None

#         # Milestone F1: store last known pose landmarks per frame
#         self.last_landmarks     = None  # updated every frame pose is detected
...
#     # ------------------------------------------------------------------
#     # Milestone F1: which foot is closer to the ball?
#     # ------------------------------------------------------------------

#     def _identify_foot(self, ball_cx, ball_cy):
#         """
#         Compare ball position to left/right ankle + foot landmarks.
#         Returns 'L', 'R', or 'U' (unknown if pose not available).

#         Uses the closest of:
#           Left  ankle (27) + left foot (31)
#           Right ankle (28) + right foot (32)
#         """
...
#         # Safely get landmark positions
#         def get_lm(idx):
#             if idx < len(lms):
#                 return np.array([lms[idx][0], lms[idx][1]], dtype=np.float32)
#             return None

#         left_ankle = get_lm(POSE_LEFT_ANKLE)   # 27
#         left_foot  = get_lm(POSE_LEFT_FOOT)    # 31
#         right_ankle = get_lm(POSE_RIGHT_ANKLE) # 28
#         right_foot  = get_lm(POSE_RIGHT_FOOT)  # 32

#         ball = np.array([ball_cx, ball_cy], dtype=np.float32)

#         # Average ankle + foot for each side (more robust than single point)
#         def side_dist(a, b):
#             pts = [p for p in [a, b] if p is not None]
#             if not pts:
#                 return float("inf")
#             center = np.mean(pts, axis=0)
#             return float(np.linalg.norm(ball - center))
...
#         """
#         F2: Gate check — returns True only if ball is within
#         FOOT_PROXIMITY_PX of either foot/ankle.
#         Prevents false touches when ball is far from any foot.
#         """
#         if self.last_landmarks is None:
#             return True  # no pose data — don't block, let it through

#         lms  = self.last_landmarks
#         ball = np.array([ball_cx, ball_cy], dtype=np.float32)

#         # Check all 4 foot-related landmarks
#         foot_indices = [
#             POSE_LEFT_ANKLE,   # 27
#             POSE_LEFT_FOOT,    # 31
#             POSE_RIGHT_ANKLE,  # 28
...
#                     return True  # ball is close enough to this landmark

#         return False  # ball too far from all foot landmarks
...
#         # Attaches foot label (L/R/U) from current pose landmarks.
#         # """
...
#         #         # F1: identify which foot touched the ball
...
#             if self.prev_vy is not None:
#                 print(f"  DEBUG t={t1:.2f}s | "
#                     f"prev_vy={self.prev_vy:.1f} | "
#                     f"vy={vy:.1f} | "
#                     f"sign_flip={(self.prev_vy > 0) and (vy < 0)} | "
#                     f"strong={abs(self.prev_vy) > self.vy_min_flip or abs(vy) > self.vy_min_flip} | "
#                     f"far={t1 - self.last_touch_t >= self.min_touch_interval} | "
#                     f"near_foot={self._is_ball_near_foot(det_point['x'], det_point['y'])}")
...
#                 if sign_flip and strong_enough and far_enough and near_foot:  # ← add near_foot
#                     foot = self._identify_foot(det_point["x"], det_point["y"])
#                     touch = {
#                         "frame_idx": det_point["frame_idx"],
#                         "t":         det_point["t"],
...
#                         f"at t={t1:.2f}s frame={det_point['frame_idx']} "
#                         f"vy_prev={self.prev_vy:.1f} vy={vy:.1f} "
#                         f"near_foot={near_foot}")  # ← helpful for tuning
#                 elif sign_flip and strong_enough and far_enough and not near_foot:
#                     # Log rejected touches so you can tune FOOT_PROXIMITY_PX
#                     print(f"  ✗ Rejected touch at t={t1:.2f}s — ball too far from foot")
...
#     # ------------------------------------------------------------------
#     # Milestone F1: L/R chart overlay (bottom-left corner)
#     # ------------------------------------------------------------------

#     def _draw_foot_chart(self, frame):
#         """
#         Draw a live L vs R touch bar chart in the bottom-left corner.
#         Green = Left, Blue = Right, Grey = Unknown
#         """
#         h, w = frame.shape[:2]
...
#         # ── Pose estimation — update last_landmarks every frame ──────
#         landmarks_px = self.detect_pose(frame, timestamp_ms)
#         if landmarks_px is not None:
#             self.last_landmarks = landmarks_px  # F1: keep latest pose
...
#                 # F1: color flash by foot — green=left, blue=right, grey=unknown
#                 flash_color = {
#                     "L": (0, 200, 0),
#                     "R": (200, 100, 0),
#                     "U": (180, 180, 180)
...
#                 cv2.putText(
#                     frame,
#                     f"TOUCH #{touch_count} [{foot}]",
#                     (tx + 15, ty - 15),
#                     cv2.FONT_HERSHEY_SIMPLEX, 0.9, flash_color, 2
#                 )

#         # F1: live L vs R bar chart
...
#         """Returns dict with L, R, U counts and percentages."""
#         feet  = [t["foot"] for t in self.touches]
#         total = len(feet)
#         if total == 0:
...
#         print(f"\n{'='*40}")
#         print(f"Touch count : {summary['total']}")
#         print(f"Left  foot  : {summary['L']} ({summary.get('L_pct', 0)}%)")
#         print(f"Right foot  : {summary['R']} ({summary.get('R_pct', 0)}%)")
#         print(f"Unknown     : {summary['U']}")
#         print(f"Best streak : {summary['total']}")
#         if self.touches:
#             print("\nFirst 5 touches:")
...
#         out_path = output_dir / f"{video_path.stem}_F1_foot_annotated.mp4"
